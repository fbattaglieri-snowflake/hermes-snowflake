...
This file documents every known gotcha encountered during deployment and operation.

## Container and SPCS

### `hermes serve` starts but never listens
...
### UDP and TUN networking unavailable
...
### DERP performance
Tailscale relays through the DERP node closest to your SPCS region. Latency is acceptable for interactive use; the relay is not the bottleneck in practice. Which DERP node is selected depends on your cloud and region — check with `tailscale netcheck` inside the container rather than assuming.

### Block volume masks image content
`/root` is a block volume (see `size` in the service spec) mounted at runtime. Everything the installer writes under `/root` at build time is hidden. The image keeps a copy in `/opt/hermes-seed` and restores it with `cp -a -n` (no-clobber) at every boot. Anything that must survive a restart belongs under `/root`; anything that must not be masked belongs outside it.

## Authentication and Client Connection

### `/auth/password-login` returns 422
...
### `hermes serve --status` reports no active process
...
### `Context length exceeded (N tokens)`
...
### Telegram bot responds only `The model provider failed after retries`
...
### HTTP 401 on every model at once
This is not a Snowflake regression and not a per-model problem. It means the PAT was not injected: the command placed `CORTEX_PAT="..."` after a `&&` instead of at the start. Place the assignment first.

### Empty response with `finish_reason: length` on reasoning models
...
Model availability is **not** a property of this project: it varies by account, cloud, region, and the `CORTEX_ENABLED_CROSS_REGION` setting. A model that works on one account can return 403 or be silently remapped to an unavailable `-global` variant on another. Never assume the shipped `models.json` matches your account — regenerate it with `refresh_cortex_models.py`, which probes each name with a real call.
...
### WS ticket is single-use with a 30-second TTL
